...
// Derived from WerWolv/nx-ovlloader, which itself is derived from switchbrew/nx-hbloader.
// This variant chainloads Switch Assistant's notification overlay directly.
...
const char g_noticeText[] =
    "switch-ha-overlay-loader " VERSION "\0"
    "Switch Assistant notification overlay loader.";
...
static void appendLog(const char *message, Result rc)
{
    ...
    fprintf(file, "%llu %s rc=0x%x\n", (unsigned long long)monotonicMs(), message, rc);
    fclose(file);
}
...
static bool g_loader_failed = false;
static u8 g_proc_thread_stack[0x4000] __attribute__((aligned(0x1000)));

static void failAndSleep(const char *message, Result rc)
{
    appendLog(message, rc);
    g_loader_failed = true;
    while (true)
...
static void procHandleReceiveThread(void* arg)
{
    ...
    if (R_FAILED(rc))
        failAndSleep("process handle recv failed", rc);

    HipcParsedRequest r = hipcParseRequest(base);
    if (r.meta.num_copy_handles != 1)
        failAndSleep("process handle missing copy handle", MAKERESULT(Module_HomebrewLoader, 17));

    g_procHandle = r.data.copy_handles[0];
    appendLog("process handle received", 0);
...
static void getOwnProcessHandle(void)
{
    ...
    appendLog("process handle create session", 0);
    Handle server_handle, client_handle;
    rc = svcCreateSession(&server_handle, &client_handle, 0, 0);
    if (R_FAILED(rc))
        failAndSleep("process handle create session failed", rc);

    Thread t;
    appendLog("process handle create thread", 0);
    rc = threadCreate(&t, &procHandleReceiveThread, (void*)(uintptr_t)server_handle, g_proc_thread_stack, sizeof(g_proc_thread_stack), 0x2B, -2);
    if (R_FAILED(rc))
        failAndSleep("process handle create thread failed", rc);

    appendLog("process handle start thread", 0);
    rc = threadStart(&t);
    if (R_FAILED(rc))
        failAndSleep("process handle start thread failed", rc);

    appendLog("process handle send request", 0);
    hipcMakeRequestInline(armGetTls(),
        .num_copy_handles = 1,
    ).copy_handles[0] = CUR_PROCESS_HANDLE;

    rc = svcSendSyncRequest(client_handle);
    appendLog("process handle send request returned", rc);
    svcCloseHandle(client_handle);

    appendLog("process handle wait thread", 0);
    threadWaitForExit(&t);
    threadClose(&t);
    if (g_loader_failed || g_procHandle == INVALID_HANDLE)
        failAndSleep("process handle invalid", MAKERESULT(Module_HomebrewLoader, 16));
    appendLog("process handle done", 0);
}
...
void loadNro(void)
{
    ...
    if (R_FAILED(rc)) {
        fsFsClose(&sdmc);
        appendLog("open overlay failed", rc);
        exit(1);
    }
    appendLog("overlay opened", 0);
    ...
    {
        appendLog("bad nro magic", MAKERESULT(Module_HomebrewLoader, 5));
    ...
    appendLog("overlay jump", 0);
    nroEntrypointTrampoline((u64) entries, -1, entrypoint);
}

int main(int argc, char **argv)
{
    appendLog("loader main", 0);
    if (hosversionBefore(9,0,0))
    {
        appendLog("unsupported hos", 0);
        exit(1);
    }
    ...
    setupHbHeap();
    appendLog("heap ready", 0);
    getOwnProcessHandle();
    appendLog("process handle ready", 0);
    loadNro();
